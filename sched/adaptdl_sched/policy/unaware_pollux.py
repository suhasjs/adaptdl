...
#
# 
# Heterogeneity-oblivious version of Pollux, using a wrapper on Pollux
...
# Phoebe experimentation
from adaptdl_sched.cluster_config import ID_TO_NODENAME_MAP, NODENAME_TO_ID_MAP, CLUSTER_NUM_GPUS, BLACKLIST_NODES
from adaptdl_sched.cluster_config import alloc_to_placement_smart, get_mock_phoebe_node, fix_allocations_by_dropping
from adaptdl.sched_hints import NODE_TO_CLUSTER_MAP


class UnawarePolluxPolicy(object):
    def __init__(self):
    ...
    def optimize(self, jobs, nodes, base_allocations, node_template):
        ...
        if DEBUG_PHOEBE:
            ...
            # create vnodes (persistent ordering)
            self.node_ordering = sorted(list(nodes.keys()))
            LOG.info(f"Node ordering: {self.node_ordering}")
            # compute number of GPUs per vnode (min of all gpu_per_node)
            vnode_ngpus_per_node = min([v.resources['nvidia.com/gpu'] for v in nodes.values()])
            vnodes = {}
            for k, v in nodes.items():
                num_vnodes = v.resources['nvidia.com/gpu'] // vnode_ngpus_per_node
                for i in range(num_vnodes):
                    vname = k + "-vnode-" + str(i)
                    vnodes[vname] = copy.deepcopy(v)
                    vnodes[vname].resources['nvidia.com/gpu'] = vnode_ngpus_per_node
                    vnodes[vname].gpu_type = NODE_TO_CLUSTER_MAP[k]
            self.vnodes = vnodes
            LOG.info(f"v-nodes: {self.vnodes}")

        # run optimizer loop with vnodes
        allocs, ret2 = self.optimize_core(jobs, self.vnodes, base_allocations, node_template)
        LOG.info(f"CORE: Allocs = {allocs}")

        # strip vnode names from allocs
        new_allocs = dict()
        for k, v in allocs.items():
            new_allocs[k] = []
            for v2 in v:
                new_allocs[k].append(v2.split('-vnode-')[0])
...
class Problem(pymoo.model.problem.Problem):
    ...
    def _evaluate(self, states, out, *args, **kwargs):
        ...
        if p == 0:
            # Geometric mean
            mean = np.exp(np.sum(np.log(np.maximum(scaled_speedups, 1e-3)), axis=1) / states.shape[1])
        else:
            mean = (np.sum((scaled_speedups + 1e-3) ** p, axis=1) / states.shape[1]) ** (1.0 / p)
